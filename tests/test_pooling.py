...
def test_pool_may_tighten_without_limit():
    ...
    anchor = original_scale_of(original)
...
def test_repeated_creep_never_accumulates():
    ...
    _, final_scale = robust_centre_scale(pool)
    assert final_scale <= anchor * CFG.max_widen + 1e-9
...
def test_pool_keeps_the_recent_end_when_evicting():
    anchor = 1.0  # generous, so nothing is rejected for widening
